 ...
 * 
 * Thanks to Mark Howard and his work on the Radiator View Plugin from which
 * this was based.
 * 
 * @author jrenaut
 */
public class XFPanelView extends ListView {
	 ...
	 * @param numColumns
	 *            the number of columns to use on the layout (work in progress)
	 */
	...
	protected void submit(StaplerRequest req) throws ServletException,
		...
		try {
			...
			throw new FormException(XFPanelViewDescriptor.REFRESH_MSG,
					"refresh");
		}
	...
	public final class XFPanelEntry {
		...
		public Integer getQueueNumber() {
			// placeInQueue==null right after deserialization because it's
			// transient
			return placeInQueue == null ? null : placeInQueue.get(this.job
					.getQueueItem());
		}
			...
			if (run != null) {
				...
				if (previous != null) {
					...
					if (tests != null && prevTests != null) {
						...
						return Functions.getDiffString(currentSuccess
								- prevSuccess);
					}
				}
			}
		...
		private Run<?, ?> getLastSuccessfulFrom(Run<?, ?> run) {
			...
			while (r != null
					&& (r.isBuilding() || r.getResult() == null || r
							.getResult().isWorseThan(Result.UNSTABLE))) {
	...
	public static final class XFPanelViewDescriptor extends ViewDescriptor {
		...
		public FormValidation doCheckRefresh(StaplerRequest req) {
			return FormValidation.validatePositiveInteger(req
